...
export function useSwapCallback(
  ...
  return useCallback(async () => {
    ...
    const inputCur = trade.inputAmount.currency
    const outputCur = trade.outputAmount.currency

    const swapInfo: ExactInputSwapTransactionInfo | ExactOutputSwapTransactionInfo = {
      type: TransactionType.SWAP,
      inputCurrencyId: currencyId(inputCur),
      outputCurrencyId: currencyId(outputCur),
      ...(inputCur.isToken
        ? { inputCurrencyDecimals: inputCur.decimals, inputCurrencySymbol: inputCur.symbol ?? 'Token' }
        : {}),
      ...(outputCur.isToken
        ? { outputCurrencyDecimals: outputCur.decimals, outputCurrencySymbol: outputCur.symbol ?? 'Token' }
        : {}),
