...
import { getInsufficientFundsUserMessage } from 'utils/insufficientFundsMessage'
import { didUserReject, isInsufficientFundsError } from 'utils/userReject'
    ...
    try {
      ...
      return {
        ...
        info: {
          ...
          spender,
          amount: allowance,
          ...(cur.isToken ? { tokenSymbol: cur.symbol ?? undefined, tokenDecimals: cur.decimals } : {}),
        },
      }
    } catch (e: unknown) {
      ...
      if (isInsufficientFundsError(e)) {
        throw new Error(getInsufficientFundsUserMessage(chainId ?? undefined))
      }
      throw new Error(`${symbol} token allowance failed: ${e instanceof Error ? e.message : e}`)
    }
  }, [amount, chainId, contract, spender])
}
