...
import { getNativeCurrencySymbol, TOKEN_SHORTHANDS } from '../../constants/tokens'
...
export function queryParametersToSwapState(parsedQs: ParsedQs, chainIdForDefault?: number): SwapState {
  let inputCurrency = parseCurrencyFromURLParameter(parsedQs.inputCurrency)
  let outputCurrency = parseCurrencyFromURLParameter(parsedQs.outputCurrency)
  const typedValue = parseTokenAmountURLParameter(parsedQs.exactAmount)
  const independentField = parseIndependentFieldURLParameter(parsedQs.exactField)
  const effectiveChainId = chainIdForDefault ?? getAppChainId()

  if (inputCurrency === '' && outputCurrency === '' && typedValue === '' && independentField === Field.INPUT) {
    // Defaults to current chain's native currency (e.g. TT for TT Chain, VANA for Vana)
    inputCurrency = getNativeCurrencySymbol(effectiveChainId)
...
export function useDefaultsFromURLSearch(): SwapState {
  ...
  const effectiveChainId = chainId ?? getAppChainId()
  const parsedSwapState = useMemo(() => {
    return queryParametersToSwapState(parsedQs, effectiveChainId)
  }, [parsedQs, effectiveChainId])
