...
import { buildTotalGasQuotedNative } from './gasQuoted'
...
export class ClassicTrade extends Trade<Currency, Currency, TradeType> {
  ...
  /** Raw wei of wrapped native from router (`gasUseEstimateQuote` quote field). */
  ...
  constructor({
    ...
    this.gasUseEstimateQuoteRaw = gasUseEstimateQuoteRaw
  ...
  /** Swap leg only, denominated in wrapped native (wei from router quote). */
  public get swapGasQuotedNative(): CurrencyAmount<Currency> | undefined {
    const raw = this.gasUseEstimateQuoteRaw
    const chainId = this.inputAmount.currency.chainId
    if (!raw || !chainId) return undefined
    try {
      return CurrencyAmount.fromRawAmount(nativeOnChain(chainId).wrapped, raw)
    } catch {
      return undefined
    }
  }

  /** Total gas incl. proportional approval estimate, in wrapped native. */
  public get gasUseEstimateQuotedNative(): CurrencyAmount<Currency> | undefined {
    return buildTotalGasQuotedNative({
      gasUseEstimateQuoteRaw: this.gasUseEstimateQuoteRaw,
      chainId: this.inputAmount.currency.chainId,
      gasUseEstimateUSD: this.gasUseEstimateUSD,
      approveGasEstimateUSD: this.approveInfo.needsApprove ? this.approveInfo.approveGasEstimateUSD : undefined,
    })
  }
}
