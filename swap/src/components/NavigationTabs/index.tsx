...
export function FindPoolTabs({ origin }: { origin: string }) {
  ...
  const backTo = useMemo(() => {
    const pathname = origin.startsWith('/') ? origin : `/${origin}`
    return toWithPreservedRouterSearch(pathname, search)
  }, [origin, search])
  ...
  return (
    <Tabs>
      <RowBetween style={{ padding: '1rem 1rem 0 1rem' }} align="center">
        <StyledLink
          to={toWithPreservedRouterSearch(poolLink, location.search)}
          onClick={() => {
            if (adding) {
