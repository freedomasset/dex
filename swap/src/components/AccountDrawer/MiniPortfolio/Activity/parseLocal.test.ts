...
describe('parseLocalActivity', () => {
  ...
  it('resolves swap tokens from persisted symbol/decimals when token list is empty', () => {
    const { formatNumber } = renderHook(() => useFormatter()).result.current

    const info: ExactInputSwapTransactionInfo = {
      type: MockTxType.SWAP,
      tradeType: MockTradeType.EXACT_INPUT,
      ...
      inputCurrencySymbol: MockUSDC_MAINNET.symbol!,
      inputCurrencyDecimals: MockUSDC_MAINNET.decimals,
      outputCurrencySymbol: MockDAI.symbol!,
      outputCurrencyDecimals: MockDAI.decimals,
    }
    const details = {
      info,
      receipt: { transactionHash: '0x123', status: 1 },
    } as TransactionDetails
    const activity = transactionToActivity(details, ChainId.MAINNET, {} as ChainTokenMap, formatNumber)
    expect(activity?.currencies?.[0]?.symbol).toEqual(MockUSDC_MAINNET.symbol)
    expect(activity?.currencies?.[1]?.symbol).toEqual(MockDAI.symbol)
    expect(activity?.descriptor ?? '').not.toMatch(/Unknown/i)
  })
