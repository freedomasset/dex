...
function useGasPrice(trade: ClassicTrade) {
  const { formatNumber, formatCurrencyAmount } = useFormatter()
  const { gasUseEstimateUSD, gasUseEstimateQuotedNative, inputAmount } = trade
  const chainId = inputAmount.currency.chainId
  if (!gasUseEstimateUSD || !SUPPORTED_GAS_ESTIMATE_CHAIN_IDS.includes(chainId)) return undefined

  if (gasUseEstimateQuotedNative) {
    const nativeSymbol = getNativeCurrencySymbol(chainId)
    return formatNetworkGasEstimateLabel({
      amount: gasUseEstimateQuotedNative,
      nativeSymbol,
