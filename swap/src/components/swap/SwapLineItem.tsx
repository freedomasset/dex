...
const AutoBadgeStyled = styled(ThemedText.LabelMicro).attrs({ fontWeight: 535 })`
...
function AutoBadge() {
  ...
  const label = i18n.locale === 'zh-CN' ? '自动' : i18n.locale === 'zh-TW' ? '自動' : 'Auto'
  return <AutoBadgeStyled>{label}</AutoBadgeStyled>
}
...
function FOTTooltipContent() {
  return (
    <>
      <Trans>
        Some tokens take a fee when they are bought or sold, which is set by the token issuer. The Freedom Asset does
...
function useLineItem(props: SwapLineItemProps): LineItemData | undefined {
  ...
  const { formatNumber, formatSlippage, formatCurrencyAmount } = useFormatter()
    ...
    case SwapLineItemType.NETWORK_COST:
      ...
      return {
        ...
        Value: () => {
          ...
          const quotedNative = isClassicTrade(trade) ? trade.gasUseEstimateQuotedNative : undefined
          const nativeSymbol = quotedNative !== undefined ? getNativeCurrencySymbol(chainId) : ''
          const label =
            quotedNative !== undefined
              ...
              : formatNumber({
                  input: isClassicTrade(trade) ? trade.totalGasUseEstimateUSD : undefined,
