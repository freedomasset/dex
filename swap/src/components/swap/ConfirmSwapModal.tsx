...
function useConfirmModalState({
  ...
  const [approvalErrorDetail, setApprovalErrorDetail] = useState<string | undefined>()
  ...
  const catchUserReject = async (e: any, errorType: PendingModalError) => {
    ...
    setApprovalErrorDetail(e instanceof Error ? e.message : String(e))
  ...
  const onCancel = () => {
    ...
    setApprovalErrorDetail(undefined)
  }
    ...
    <Modal isOpen $scrollOverlay onDismiss={onModalDismiss} maxHeight={90}>
      {errorType ? (
        <ErrorModalContent
          ...
          detailMessage={
            approvalErrorDetail ??
            (errorType === PendingModalError.CONFIRMATION_ERROR && swapError ? swapError.message : undefined)
          }
        />
      ) : (
