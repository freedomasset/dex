...
describe('PendingModalContent', () => {
  ...
  it('renders correctly with only one step', () => {
    ...
    expect(screen.getByText('Enable token spending for Freedom Asset')).toBeInTheDocument()
    expect(screen.getByText('Proceed in your wallet')).toBeInTheDocument()
    expect(screen.queryByText('Why is this required?')).not.toBeInTheDocument()
  })
  ...
  describe('renders the correct step when there are multiple', () => {
    it('renders the first step with activeStepIndex=0', () => {
      ...
      expect(screen.getByText('Proceed in your wallet')).toBeInTheDocument()
      expect(screen.queryByText('Why is this required?')).not.toBeInTheDocument()
      expect(screen.queryByText('Sign to authorize swapping')).not.toBeInTheDocument()
    })
    ...
    it('renders the second step with activeStepIndex=1', () => {
      ...
      expect(screen.getByText('Sign to authorize swapping')).toBeInTheDocument()
      expect(screen.getByText('Proceed in your wallet')).toBeInTheDocument()
      expect(screen.queryByText('Why is this required?')).not.toBeInTheDocument()
      expect(screen.queryByText('Enable token spending for Freedom Asset')).not.toBeInTheDocument()
    })
