  ...
  const handleCurrencyASelect = useCallback(
    (currencyA: Currency) => {
      ...
      if (newCurrencyIdA === currencyIdB) {
        navigateWithQuery(`/add/v2/${currencyIdB}/${currencyIdA}`)
      } else {
        navigateWithQuery(`/add/v2/${newCurrencyIdA}/${currencyIdB}`)
      }
    },
    [currencyIdB, navigateWithQuery, currencyIdA]
  )
  const handleCurrencyBSelect = useCallback(
    (currencyB: Currency) => {
      ...
      if (currencyIdA === newCurrencyIdB) {
        if (currencyIdB) {
          navigateWithQuery(`/add/v2/${currencyIdB}/${newCurrencyIdB}`)
        } else {
          navigateWithQuery(`/add/v2/${newCurrencyIdB}`)
        }
      } else {
        navigateWithQuery(`/add/v2/${currencyIdA ? currencyIdA : 'ETH'}/${newCurrencyIdB}`)
      }
    },
    [currencyIdA, navigateWithQuery, currencyIdB]
  )
