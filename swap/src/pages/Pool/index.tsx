...
import { getAppChainId, isSupportedChain } from 'constants/chains'
...
export default function Pool() {
  ...
  const effectiveChainId = chainId ?? interfaceChainId
  const nativeSymbolChainId = interfaceChainId ?? chainId ?? getAppChainId()

  const newPositionLink = useMemo(
    () => toWithPreservedRouterSearch(`/add/${getNativeCurrencySymbol(nativeSymbolChainId)}`, location.search),
    [location.search, nativeSymbolChainId]
  )
    ...
    <>
      <PageWrapper>
        <AutoColumn gap="lg" justify="center">
          <AutoColumn gap="lg" style={{ width: '100%' }}>
            {showChainMismatch && (
              <YellowCard padding="12px" data-testid="pool-chain-mismatch">
                <RowBetween gap="12">
                  <ThemedText.BodySecondary fontSize={14}>
                    <Trans>
                      Your wallet is on {getChainInfo(chainId)?.label ?? '…'}. Switch to match the app network (
                      {getChainInfo(interfaceChainId)?.label ?? '…'}).
                    </Trans>
            ...
            <TitleRow padding="0">
              ...
              <ButtonRow>
                {networkSupportsV2 && (
                  <PoolMenu
                    menuItems={menuItems}
                    internalLinkSearch={location.search}
              ...
              {positionsLoading ? (
                ...
                <ErrorContainer>
                  ...
                  {!showConnectAWallet && closedPositions.length > 0 && (
                    <ButtonText
                      style={{ marginTop: '.5rem', color: '#4040ff' }}
