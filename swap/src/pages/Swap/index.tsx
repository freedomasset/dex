...
import transformIcon from '../../assets/images/simpleFlow/transform.png'
    ...
    <>
      <PageWrapper>
        <Swap
          ...
          disableTokenInputs={interfaceChainId === undefined}
        />
        {/* <NetworkAlert /> */}
      </PageWrapper>
...
function Swap({
  ...
  const handleDismissTokenWarning = useCallback(() => {
    setDismissTokenWarning(true)
    navigate(toWithPreservedRouterSearch('/swap/', routerSearch))
  }, [navigate, routerSearch])
  ...
  const handleOutputSelect = useCallback(
    (outputCurrency: Currency) => {
      ...
      // 如果当前用户在 input 字段输入了值，选择 output 代币时不清空 input 的值
      // 只清空 output 的值，但保持 independentField 为 INPUT
      if (independentField === Field.INPUT && typedValue) {
        // 保持 input 的值不变，只更新代币
        // output 的值会通过 trade 重新计算
      } else {
        // 如果当前没有输入值或 independentField 是 OUTPUT，则清空 output
    ...
    [onCurrencyChange, onCurrencySelection, onUserInput, state, independentField, typedValue]
  )
      ...
      <div style={{ display: 'relative' }}>
        ...
        <ArrowWrapper clickable={isSupportedChain(chainId)}>
          ...
          >
            ...
            <img src={transformIcon} alt="transform" style={{ width: '40px', height: '40px' }} />
          </ArrowContainer>
        </ArrowWrapper>
      </div>
      <AutoColumn gap="30px">
        ...
        <div>
          {swapIsUnsupported ? (
            <ButtonPrimary $borderRadius="24px" disabled={true}>
            ...
            <ButtonLight onClick={toggleWalletDrawer} fontWeight={535} $borderRadius="24px">
