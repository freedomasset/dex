      ...
      await router[methodName](...args, {
        ...
        .then((response: TransactionResponse) => {
          ...
          addTransaction(response, {
            ...
            baseCurrencySymbol: currencyA.symbol ?? undefined,
            baseCurrencyDecimals: currencyA.decimals,
            quoteCurrencySymbol: currencyB.symbol ?? undefined,
            quoteCurrencyDecimals: currencyB.decimals,
          })
  ...
  const handleSelectCurrencyA = useCallback(
    (currency: Currency) => {
      if (currencyIdB && currencyId(currency) === currencyIdB) {
        navigate(toWithPreservedRouterSearch(`/remove/v2/${currencyId(currency)}/${currencyIdA}`, location.search))
      } else {
        navigate(toWithPreservedRouterSearch(`/remove/v2/${currencyId(currency)}/${currencyIdB}`, location.search))
      }
    },
    [currencyIdA, currencyIdB, navigate, location.search]
  )
  const handleSelectCurrencyB = useCallback(
    (currency: Currency) => {
      if (currencyIdA && currencyId(currency) === currencyIdA) {
        navigate(toWithPreservedRouterSearch(`/remove/v2/${currencyIdB}/${currencyId(currency)}`, location.search))
      } else {
        navigate(toWithPreservedRouterSearch(`/remove/v2/${currencyIdA}/${currencyId(currency)}`, location.search))
      }
    },
    [currencyIdA, currencyIdB, navigate, location.search]
  )
