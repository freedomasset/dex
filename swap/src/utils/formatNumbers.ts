...
/** Truncate to N decimal places (discard remainder, no rounding). */
function truncateToFractionDigits(value: number, fractionDigits: number): number {
  if (fractionDigits <= 0 || !Number.isFinite(value)) return value
...
function formatNumber({
  ...
  // For TokenNonTx (e.g. balance), truncate to max fraction digits instead of rounding
  if (type === NumberType.TokenNonTx && !hardCodedInput && typeof formatterOptions.maximumFractionDigits === 'number') {
    input = truncateToFractionDigits(input, formatterOptions.maximumFractionDigits)
  }
...
// Use enough significant digits so balance decimals are not truncated before formatter rounding (SDK default is 6).
const CURRENCY_AMOUNT_SIGNIFICANT_DIGITS = 18
...
function formatCurrencyAmount({
  ...
  return formatNumber({
    input: amount ? parseFloat(amount.toSignificant(CURRENCY_AMOUNT_SIGNIFICANT_DIGITS)) : undefined,
    type,
    placeholder,
    locale,
    localCurrency,
