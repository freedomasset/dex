...
export function usePoolDatas(poolAddresses: string[]): {
  ...
  const {
    ...
    error: error24,
    data: data24,
  } = useQuery<PoolDataResponse>(POOLS_BULK(safeBlock24, poolAddresses), { 
    client: dataClient,
  ...
  const {
    ...
    error: error48,
    data: data48,
  } = useQuery<PoolDataResponse>(POOLS_BULK(safeBlock48, poolAddresses), { 
    client: dataClient,
    skip: !safeBlock48,
  })
  const {
    ...
    error: errorWeek,
    data: dataWeek,
  } = useQuery<PoolDataResponse>(POOLS_BULK(safeBlockWeek, poolAddresses), { 
    client: dataClient,
    skip: !safeBlockWeek, // 如果区块号无效，跳过查询
  })
