...
export function useAllTokenData(): {
  ...
  // Filter tokens based on TOKEN_ALLOW_LIST (如果列表为空则不过滤)
  return useMemo(() => {
    const allowList = TOKEN_ALLOW_LIST[activeNetwork.id] || []
    
    // 如果 ALLOW_LIST 为空，返回所有代币
...
export function usePoolsForToken(address: string): string[] | undefined {
  ...
  useEffect(() => {
    async function fetch() {
      ...
      if (!loading && !error && addresses) {
        // 如果 ALLOW_LIST 为空则不过滤，否则只返回在列表中的池子
        const allowList = POOL_ALLOW_LIST[activeNetwork.id] || []
        const filteredAddresses = allowList.length > 0
          ? addresses.filter((addr) => allowList.includes(addr.toLowerCase()))
          : addresses
        dispatch(
          addPoolAddresses({
  ...
  // 如果 ALLOW_LIST 为空则不过滤
  const allowList = POOL_ALLOW_LIST[activeNetwork.id] || []
  return allowList.length > 0
    ? poolsForToken?.filter((addr) => allowList.includes(addr.toLowerCase()))
    : poolsForToken
}
