...
const DataRow = ({ tokenData, index }: { tokenData: TokenData; index: number }) => {
  ...
  return (
    <LinkWrapper to={toWithPreservedRouterSearch(`/tokens/${tokenData.address}`, search)}>
      <ResponsiveGrid>
        <Label>{index + 1}</Label>
        {/* @ts-ignore */}
        <Label>
          <RowFixed>
            <ResponsiveLogo address={tokenData.address} symbol={tokenData.symbol} />
          </RowFixed>
...
export default function TokenTable({
  ...
  const sortedTokens = useMemo(() => {
    const allowList = TOKEN_ALLOW_LIST[currentNetwork.id] || []
    return tokenDatas
      ? tokenDatas
  ...
  // 如果数据为空且不在加载中，显示空状态
  if (sortedTokens.length === 0 && !isLoading) {
    return (
      <Wrapper>
        ...
        <AutoColumn $gap="16px">
          <ResponsiveGrid>
            ...
            <ClickableText color={theme?.text2} end={1} onClick={() => handleSort(SORT_FIELD.priceUSDChange)}>
              {t('tablePriceChange')} {arrow(SORT_FIELD.priceUSDChange)}
            </ClickableText>
