...
import SearchIcon from 'assets/images/simpleFlow/search.png'
...
const OptionButton = styled.div<{ $enabled: boolean }>`
  ...
  align-items: center;
  background-color: ${({ theme, $enabled }) => ($enabled ? '#182127' : 'transparent')};
  color: ${({ theme, $enabled }) => ($enabled ? '#00FFF1' : '#7E98A7')};
  :hover {
    opacity: 0.6;
    cursor: pointer;
  }
...
const Search = ({ ...rest }: React.HTMLAttributes<HTMLDivElement>) => {
  ...
  const handleNav = (to: string) => {
    ...
    setPoolsShown(3)
    setTokensShown(3)
    navigate({ pathname: to.startsWith('/') ? to : `/${to}`, search })
  }
  ...
  return (
    <Hotkeys keyName="command+/" onKeyDown={handleDown}>
      ...
      <Container>
        <Wrapper {...rest}>
          <img src={SearchIcon} alt="search" style={{ width: '20px', height: '20px', marginRight: '5px' }} />
          <StyledInput
            type="text"
            value={value}
            onChange={(e) => {
        ...
        <Menu $hide={!showMenu} ref={menuRef}>
          <AutoColumn $gap="lg">
            ...
            {tokensForList
              .filter((t) => {
                const allowList = TOKEN_ALLOW_LIST[activeNetwork.id] || []
                return allowList.length === 0 || allowList.includes(t.address.toLowerCase())
              })
              .slice(0, tokensShown)
              .map((t, i) => {
                return (
                  <HoverRowLink onClick={() => handleNav('/tokens/' + t.address)} key={i}>
                    <ResponsiveGrid>
                      <RowFixed>
                        <CurrencyLogo address={t.address} symbol={t.symbol} />
                        <TYPE.label ml="10px">
                          {/* @ts-ignore */}
                          <>
            ...
            {tokensForList.length === 0 ? (
              <TYPE.main>{showWatchlist ? t('savedTokensWillAppearHere') : t('noResults')}</TYPE.main>
            ) : null}
            <HoverText
              onClick={() => {
            ...
            {poolForList
              .filter((p) => {
                const allowList = POOL_ALLOW_LIST[activeNetwork.id] || []
                return allowList.length === 0 || allowList.includes(p.address.toLowerCase())
              })
              .slice(0, poolsShown)
              .map((p, i) => {
                return (
                  <HoverRowLink onClick={() => handleNav('/pools/' + p.address)} key={i}>
                    <ResponsiveGrid key={i}>
                      ...
                      <HideSmall>
                        <TYPE.label style={{ float: 'right', maxWidth: '100px', textAlign: 'right', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} textAlign="end">{formatDollarAmount(p.token0Price)}</TYPE.label>
                      </HideSmall>
                    </ResponsiveGrid>
                  </HoverRowLink>
                )
            ...
            {poolForList.length === 0 ? (
              <TYPE.main>{showWatchlist ? t('savedPoolsWillAppearHere') : t('noResults')}</TYPE.main>
            ) : null}
            <HoverText
              onClick={() => {
