...
import { Token, CurrencyAmount } from '@vanadex/sdk-core'
      ...
      if (poolTickData) {
        const newData = await Promise.all(
          poolTickData.ticksProcessed.map(async (t: TickProcessed, i) => {
            // Validate tick index is within valid range
            const validTickIdx = Math.max(TickMath.MIN_TICK, Math.min(TickMath.MAX_TICK, t.tickIdx))
            
            const active = t.tickIdx === poolTickData.activeTickIdx
            let sqrtPriceX96
            try {
              sqrtPriceX96 = TickMath.getSqrtRatioAtTick(validTickIdx)
            } catch (error) {
              console.error('Invalid tick index:', t.tickIdx, error)
              return null
            }
            const feeAmount: FeeAmount = poolData.feeTier
            const mockTicks = [
              {
                index: Math.max(TickMath.MIN_TICK, Math.min(TickMath.MAX_TICK, t.tickIdx - TICK_SPACINGS[feeAmount])),
            ...
            const pool =
              token0 && token1 && feeTier
                ? new Pool(token0, token1, feeTier, sqrtPriceX96, t.liquidityActive, validTickIdx, mockTicks)
                : undefined
            let nextSqrtX96
            if (poolTickData.ticksProcessed[i - 1]) {
              const prevTickIdx = Math.max(TickMath.MIN_TICK, Math.min(TickMath.MAX_TICK, poolTickData.ticksProcessed[i - 1].tickIdx))
              try {
                nextSqrtX96 = TickMath.getSqrtRatioAtTick(prevTickIdx)
              } catch (error) {
                console.error('Invalid previous tick index:', poolTickData.ticksProcessed[i - 1].tickIdx, error)
                nextSqrtX96 = undefined
              }
            } else {
              nextSqrtX96 = undefined
            }
        ...
        // Filter out null entries (invalid ticks)
        const validData = newData.filter((entry): entry is ChartEntry => entry !== null)
        // offset the values to line off bars with TVL used to swap across bar
        validData?.map((entry, i) => {
          if (i > 0) {
            validData[i - 1].tvlToken0 = entry.tvlToken0
            validData[i - 1].tvlToken1 = entry.tvlToken1
          }
        })
